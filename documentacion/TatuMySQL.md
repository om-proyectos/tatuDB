...
`tatuMySQL` es la implementación del motor MySQL para TatuDB. Provee métodos genéricos para ejecutar consultas SELECT, INSERT, UPDATE, DELETE y funciones almacenadas, utilizando un formato de entrada estandarizado heredado de `tatuGenerico`.
...
```php
require_once 'tatuDB/MySQL/tatuMySQL.php';
...
|-----------|------|-------------|
| `servidor` | string | Host o IP del servidor MySQL |
| `puerto` | int | Puerto TCP del servicio MySQL (default: 3306) |
...
Ejecuta una función almacenada de MySQL y retorna su código de salida.
...
|-----------|------|-------------|
| `$servidor` | protected | Host o IP del servidor MySQL |
| `$puerto` | protected | Puerto TCP del servicio MySQL |
